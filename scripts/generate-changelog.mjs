...
const DOCS_CHANGELOG_PATH = resolve(ROOT, 'docs', 'changelog.md');

const REPO_OWNER = 'nazmul-nhb';
const REPO_NAME = 'bn-calendar';
...
async function main() {
	...
	if (hasUnreleased) {
		...
		if (firstAfter) {
			...
			if (notes) {
				sections.push(
					[
						`## [Unreleased/Latest](${REPO_URL}/compare/${latestTag}...HEAD)`,
						'',
						notes,
					].join('\n')
				);
	...
	writeFileSync(DOCS_CHANGELOG_PATH, changelog, 'utf-8');
}
